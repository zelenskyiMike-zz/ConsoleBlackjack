        ...
        public static int BuyChips()
        {
            ...
            if (regex.IsMatch(inputValue))
            {
                ...
                if (money >100 && money <= 500)
                {
                    ...
                    /* If we want to sell maximum chips to a client- 
                     * we have to check if we can give him chips with lower denomination*/

                    //if (money % chipsValue != 0)
                    //    Console.Write("and {0} chips with a face value of {1} dollars", (money % chipsValue)/(chipsValue / 2), chipsValue/2);
                }
